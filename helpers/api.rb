  ...
  module Helpers
    module API
      ...
      def me(api_key)
        ...
        response = RestClient.get(Applyance::Admin.settings.api_host + '/accounts/me', headers) { |response, request, result| response }
        JSON.parse(response)
      end

      def authenticate(params)
        response = RestClient.post(Applyance::Admin.settings.api_host + '/accounts/auth', JSON.dump(params), { :content_type => 'application/json'}) { |response, request, result| response }
